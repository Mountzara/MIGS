...
def _num_tokens(text: str) -> set:
    ...
    return {x.replace(",", "") for x in re.findall(r"\d[\d,]*(?:\.\d+)?", t)}
...
def prose_faults(W: str, h: str, man: dict) -> list:
    ...
    for q in man["pmids"]:
        ...
        # the section's own <h3> ("Verbatim PubMed abstract") is not part of the
        # abstract; counting it made every correctly written abstract look wrong
        sec_body = re.sub(r"<h[1-6][^>]*>[\s\S]*?</h[1-6]>", " ", sec.group(1)) if sec else ""
        if not sec or alnum(src) not in alnum(sec_body):
