...
| `relocate_mid_sentence_markers` | a marker a published brief still carries from the old surname placer ("Pan's¹ review…", or "…preserved¹.") is moved to the end of its sentence and merged with any marker already there | the transform audit refused W33 for a marker inside a noun phrase; the live body had all 86 placed that way |
| `verify_design_tags` | every card's study-design badge is checked against the paper's abstract (model, fixed `DESIGN_VOCAB`, `cache.design.json`); a wrong one is rewritten | a narrative review was badged "Retrospective Cohort" |
...
| `review_inserted_citations` → `correct_unsupported_sentences` | a per-citation model check on the exact sentence the marker ends (popovers masked; neighbours labelled context only; the other papers cited on the same sentence listed, so each marker is judged for ITS part). Wrong paper ⇒ marker withdrawn **by position**. Right paper but misstated ⇒ the sentence is rewritten from the abstract (one rewrite per sentence, `cache.fix.json`), keeps its citation, and is reviewed again — still wrong refuses the brief | withdrawing by PMID removed fifteen correct instances of a paper cited once wrongly; handing the reviewer the 320 unmasked characters before the marker gave it the previous paper's hover card as "the sentence" and withdrew 27 correct citations in a chain |
...
| **`fix_deep_dive_attribution`** | a deep dive's editorial text credits the dialog's own paper, deterministically: first-author surname for "X et al.", "the X cohort", "X's finding". No model — a writer's general rules were rejecting the correction because the passage already broke one of them elsewhere | W21's dialog for the Lv paper said "the Tian cohort" four times; nothing read dialogs, and dialogs are now masked from every prose pass |
| `normalize_legacy_markup`: bracketed names | `[Tian, <marker>]` — a surname bracketed around a real marker, the pre-marker way of naming a paper — becomes the marker alone | neither a marker nor an "et al.", so no pass read it; W21 refused twice over a name that only existed inside the bracket |
| `body_invariant_faults`, three more | a brief that cards or cites nothing; a wrapper emptied beside its own text; a surname bracketed around a marker | every invariant was vacuously true of a hollow page; the other two shipped and were caught only by the model audit |
...
| **`_after_run`** / `_RUN_BREAK_RE` / **`_marker_runs`** | a run of stacked markers on a PUBLISHED body has zero-width break opportunities between its markers (put there by `breakable_marker_runs` so the run wraps on a phone). Every run-walker — the site audit, the audit-stage repairs, the deterministic citation step — used to stop at the first one and see one marker of a stack. One walker now, stepping over breaks; `_marker_runs` groups the marker matches by position | two placement passes each saw "one marker", decided the paper was uncited, and appended it again: "…translation.⁵ ⁵" |
| **`dedupe_run_markers`** | within one run, a paper is cited once — the later marker and the break before it come out, nothing else is touched. Runs before every renumber and at every resume | the first version matched "a whole run" with one non-greedy regex; it backtracked through everything between two NON-adjacent markers, cards included, and the run it rebuilt dropped sixty-four of W21's cards. The invariant gate refused it and the checkpoint was intact — that is what the gate is for |
| **`_renumber_if_unnumbered`** | a marker showing a PMID has not been through numbering; numbering (with `renumber_card_badges` and `build_references`) is re-run at audit entry, after any audit-stage supply, and unconditionally at a `--from=numbered` resume — so a marker is never on the page without its number and its reference | the audit-stage supply placed twenty-five raw-PMID markers on W21 after numbering had run, and nothing downstream numbered them |
| **`audit_transform`: supply and withdraw** (`_MISSING_CITE_RE`, `_WRONG_CITE_RE`) | the repair loop only writes prose, so a defect that names a MISSING citation ("cites only 6 distinct", "never receives a marker", "uncited") or a MISPLACED one burned every round and refused the brief. Now a missing-citation defect re-runs the placement chain (`cite_missing_studies` → `cite_named_unique` → `cite_every_card` → `cite_uncited_cards`), reviews what it placed, renumbers, and reads the page AGAIN (`before_supply`); a wrong-marker defect withdraws by position through the reviewer and renumbers. The repair prompt is handed each cited paper's AUTHORS so a wrong name is corrected from the byline, not guessed | W20 was refused over fifteen carded papers no sentence cited; nothing in the audit stage could place a marker |
| **`fix_prose_attribution`** | reader prose credits the paper it cites by that paper's own authors — forms "X et al.", "X's finding/OR", "X 2026", "the X study/cohort/trial/signal", "X Cochrane". Authority is the sentence's OWN cited papers' bylines (`_near_surname` tolerance; `_NOT_A_SURNAME` keeps articles, months, days, cities and journal words out). A sentence stating a PMID or an editorial notice (expression of concern, retraction) is left alone. **When the sentence cites exactly one paper and names exactly one wrong surname, the first author is substituted without asking a model.** Runs in `_renumber` stage 0 before the narrative rewrite, and again at resume | "Tian et al., n = 255" on a marker to Lv; "Beni" for Kizildemir; "Liu" for Shen; "Yang et al." for Guzelbag on W21 — where the model-mediated version refused the fix twice for reasons elsewhere in the passage |
| **`cite_named_unique`** | "X et al." / "X and colleagues" naming a paper the brief holds whose first author is X and no other held paper's — gets that paper's marker. No judgement needed. Runs LAST in `cite_and_review`, just before the `cite_uncited_cards` backstop | "fezolinetant … (Lederman et al.)" carried a marker only to the NAMS statement named earlier in the sentence |
| **`withdraw_citations_from_absence_claims`** + `_asserts_no_evidence` | a sentence whose claim is that no evidence exists ("no RCTs", "absence of adequately powered trials") carries no marker; `cite_prose` will not place one there either | a marker on "there are no randomized trials" points the reader at a paper as proof of nothing |
| **`drop_process_commentary`** (`_PROCESS_TALK_RE`) | a reader-prose sentence about the pipeline — "classification artifact", "keyword over-match", "the retrieval bucket" — is deleted; also a reader-gate fault and a body invariant | a published brief explained to its reader why the pipeline had pulled a paper |
| `drop_bracket_residue` | `[Bafort 2020, ]` — a legacy bracket whose marker a later pass withdrew — goes. The comma is REQUIRED: without it the pattern matched ordinary bracketed text on eleven briefs | |
| `mend_stranded_text` · `repair_split_tags` · `drop_empty_list_items` · `fix_pyramid_bars` · `refresh_deep_dive_meta` | the deterministic markup repairs `normalize_legacy_markup` runs on a body before anything reads it: text stranded beside an emptied `<span class="mz-rec-text">` goes back inside; `<span class="x"Text…>` gets its `>`; an emptied `<li>` (and an emptied list) goes; a pyramid bar is drawn from its printed count (a zero draws nothing); the deep-dive modal's author/journal/year line is rebuilt from the PubMed record like every other place the paper is named | each of these was found on a published page |
| **`own_text`** / `_paper_text_spans` | the reader's surface split the way the page is: the SITE's own text (prose, syntheses, ledes, intros, headings, card lens paragraphs, popover findings, and the deep-dive dialogs' editorial sections) versus the PAPER's own words (verbatim abstracts, card and modal titles and meta lines, popover title/meta, the reference list) plus scripts, styles, comments. Every "the site's own prose" rule reads this now — S10 terminology, S9 provenance — instead of `prose_fragments` (which never held dialogs) or `_vis_text` (which held the papers' words) | the deploy's live audit refused W34, W33 and W28 for bare MIGS inside dialog text no prose gate read; then W34's resume was refused for "AI-generated" inside a PCOS paper's own abstract |
| **`canonical_practice_name`** | "CBG/MIGS" written in the site's own TEXT NODES wherever a bare "MIGS" or a wrong-order "MIGS & CBG" stands — never in a tag, attribute (`class="mz-post-do-migs"`), script, style, comment or paper span; idempotent; runs at renumber stage 0, at a numbered resume, after the audit repair, and in `finish_and_audit`. Proven on all 16 live bodies: 39 hits → 0, no marker, card, id, class or paper text changed | W34 22×, W33 16×, W28 1× — shipped, because the pipeline's S10 gate and the deploy's §1.2 gate disagreed about what the page was |
| **`practice_name_faults`** (in `body_invariant_faults`) | the deploy's own test (BARE_MIGS / WRONG_ORDER, verbatim) on the site's own text is a body invariant, so the pre-publish gate and `audit_published_briefs.py` refuse what the deploy would refuse. A paper whose own words say "MIGS" is a printed NOTE, not a fault: the quote may not be rewritten, and the run must not publish into the deploy's refusal without saying why | |
| **`fix_invented_experience`** | a sentence that speaks from the clinician's own patients ("In my practice I found…", `EXPERIENCE_RE`) is rewritten from the paper it cites through `repair_from_defects`, abstracts in hand; `writer_reject` stops a writer producing one, this repairs the ones already on the page. Same four call sites as the practice name | W34's Endocrine synthesis: "In my practice I found it notable that twelve weeks of Nigella sativa oil lowered serum IL-6…" — a trial result in the voice of a witness; the gate refused it and nothing repaired it |
| **`_confirm_numeric_claims`** | every blocking defect from the read-back that says one figure contradicts another goes to a SECOND reader with the exact sentences, who must write the arithmetic out before answering, default refuted; an unconfirmed claim is demoted to cosmetic and printed, never repaired. Defects the ratio check computed in code are not re-asked | W28's read-back called "16, then 21 to 23 — a five-to-seven-year delay" an arithmetic contradiction and the repair loop rewrote a correct sentence on its word (it happened to rewrite it to itself) |
| `numeric_consistency_defects` (c) + **`_ratio_arithmetic_defects`** | the numeric pass was told to report only two fault classes; a derived figure — a gap, share, sum, range or change the figures it comes from do not produce — was a third and went unasked. It is asked now, with the arithmetic shown; and a share written beside its numerator and denominator ("12 of 47 (31%)") is checked by division in code, no reader involved | |
| **`canonical_post_fields`** / `post_field_faults` / `canonical-fields <id>` | the deploy's live audit scans the post's title, summary, verdict and social drafts as well as the body; every PUT site corrects those fields with `canonical_practice_name_text` ("#MIGS" → "#CBGMIGS", kept once) and refuses a field that still fails the deploy's own test. `canonical-fields` does the fields alone | W34 and W33 failed the deploy's audit on "#MIGS" in both social drafts after their bodies were clean; the pipeline had never read those fields and its PUT carried only the body |
| S7 in `grounding_audit`; S16 in `ai_review` | a study's dose is never a grounding fault on a brief (recorded, not counted; a dose phrased as an instruction is caught as advice); two verifier prompts that still carried the reversed rule state the current one. A reviewer's blocking finding is deferred to a later stage only when its LEADING label names other stages' standards — "see Table S2" mid-sentence never defers | the audit failed a study's dose on the narrative, the lede and every synthesis — the surfaces S7 names as legitimate; a stage could pass on its own finding because its wording mentioned another standard |
| **audit before the gates** (`finish_and_audit`, `_renumber`) | `audit_transform` — the S16 read-back with its repair loop — runs BEFORE the post-conditions, so `prose_faults`, the grounding audit and every deterministic gate judge the body that is written; `_number_final_page` recomputes the citation order from that page. The separate "after the audit repair" refusal is gone: the post-conditions cover it | the audited body was not the published body: the read-back's repairs rewrote prose and inserted citations after every gate had passed, and nothing read the result |
| **renumber enforces every standard** (`_manifest_of_page`, `require_spec_review`) | the republish path builds the page's manifest (carded pmids, format from the post's kind, topics from the headings), runs `prose_faults` (S5 verbatim abstracts, S12 TOC, S15 deep-dive sections, the sentence and card checks) and, for a trend brief, `trend_format_faults`, and requires the standards receipt like every other publish | `renumber` ran `reader_prose_faults` alone and skipped the receipt, so seven trend briefs republished carrying a verdict gauge S13 forbids, and a stage could publish without the pipeline having been reviewed against THE STANDARDS |
| S4: cited in the PROSE | a carded paper counts as cited only by a marker outside the deep-dive dialogs (`body_invariant_faults`, `finish_and_audit`) | a paper whose only marker sat inside another paper's deep dive passed a check meant to require citation in flowing prose |
| **`retire_verdict_gauge`** · **`trend_format_faults`** | the gauge element (and any "REVIEW REQUIRED" label) is removed at renumber stage 0 and at resume; `trend_format_faults` reads a finished trend body for the gauge, scoring language ("verdict", "debunk", "myth", "misinformation", "influencer") in the site's own text — dialogs included — the missing "Where the two sides can meet" section and the missing framing label from the fixed list | all seven published trend briefs carried the gauge; "verdict" stood in one h2 and in dialog text, "influencer" in two ledes; none had the bridge section or a framing label |
| **`conform_trend_brief`** | the model-driven S13 repair for a published legacy trend brief, run before the citation chain so what it writes is cited, reviewed and numbered by the one chain: every scoring sentence and heading rewritten from the cited abstracts with its markers kept in place (⟦PMID⟧ tokens, `_replace_span`), the bridge section authored in the brief's own voice under `BRIDGE_TONE` and adversarially verified (default refute, three attempts), one framing label chosen from `FRAMINGS`; paper text and every marker outside the bridge asserted unchanged; every verdict cached. A numbered checkpoint that still lacks the bridge or the label is refused with the resume point named (`--from=curated`) | proven on the live MHT brief: four sites rewritten, the bridge authored and verified, the framing chosen, nothing else touched |
...
`resynth`, `design`, `fix`, `transform`, plus `kb` for the heading-area lookups. A regex fix after a failed check reruns the whole
...
| **`cite_and_review`** = `refresh_card_abstracts` → `remove_orphan_studies` (confirmed against the most similar covered papers) → `relocate_mid_sentence_markers` → `verify_design_tags` → `cite_prose` (every study a sentence reports, 30-sentence chunks) → `cite_named_studies` → `cite_every_card` (every card cited in ITS section's synthesis, or one sentence written from its abstract) → `cite_missing_studies` (a second look at each sentence with its citations visible; a stated count is honoured) → `fix_stated_counts` ("N ⟨topic⟩ papers" = every card the section holds; "two reviews" = by kind; a rewrite that leaves the count unchanged is re-asked, then refused) → `review_inserted_citations` (omitted items re-asked; a wrong description of the paper's subject is unsupported) → `correct_unsupported_sentences` (two rounds, the second a different question; the ABSTRACT is the authority when a title disagrees with it, and the discrepancy is named) → re-review | `finish_and_audit`, before numbering | `_renumber` |
| `normalize_card_ids` → `breakable_marker_runs` (a zero-width space between consecutive markers so a fifteen-marker run wraps on a phone instead of falling off the clipped edge; stripped again by `normalize_legacy_markup`) | `finish_and_audit`, before numbering | stage 2 of `_renumber` |
...
**WHAT A RESUME RE-RUNS (2026-09-20).** `renumber <id> --from=numbered`
replays the last checkpoint, and a checkpoint banks the body as it was
when a pass that has since been fixed wrote it. So the resume path is not
"continue": before the audit it runs `dedupe_run_markers`,
`drop_process_commentary`, `fix_prose_attribution` and a full renumber
(badges and deep-dive labels included), and banks the result back as the
`numbered` checkpoint. A deterministic repair added today applies to a
brief checkpointed yesterday without a fresh run. `audit_transform` then
reads the body as it is, and `body_invariant_faults` is what refuses it —
neither cares which pass was supposed to have held the invariant.
