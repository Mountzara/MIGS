...
| `licensure.js` | `getLicensedStates(env)`, `isLicensedInState(env, state)`, `recordLicensureBlock(env, {patient_id, state, reason})`, `_resetCache` | The 3 R3 state-gates — `intake/[id]/submit.js`, `intake/[id]/triage.js`, `appointments/book.js` — plus `/api/v1/admin/practice/licensed-states.js` (write side) + `admin/scheduling/index.html` (picker UI) | **Phase 17 R3.** Reads `practice_settings.licensed_states_json` (keyed by clinician); 60 s cache; conservative `["IL"]` fallback (fails closed). Patient state of residence comes from intake **Section 1 `address_state`** (a `<select>` in `portal/intake/index.html`, stored schemaless in `intake_section_data`). Changing the storage key/shape touches all 3 gates + the admin endpoint + UI + the intake field in lock-step. |
