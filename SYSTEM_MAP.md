...
| `x12_837.js` · `claim_scrub.js` · `clearinghouse.js` | **Outbound billing rail.** `claim_scrub.scrubClaim(norm)` = clean-claim gate (hard blocks + denial-risk warnings; deterministic, unit-tested). `x12_837.generate837P(norm)` = ANSI X12 5010A1 837P EDI generator (unit-tested structure). `clearinghouse.submitClaim(env,…)` = pluggable adapter; `CLEARINGHOUSE_VENDOR` selects `mock` (DEFAULT, simulated 277CA — pipeline runs before payer enrollment), `stedi`, etc. Orchestrated by `POST /api/v1/admin/billing/claims/:id/submit`. | `claims/[id]/submit.js` | PHI on a live vendor call → keep `mock` + usage `'T'` until clearinghouse BAA + per-payer EDI enrollment are live (`CLEARINGHOUSE_LIVE=1` flips usage to `'P'`). Insurance fields (member id, gender, billing address) are NOT yet modeled — supplied in the submit body until an intake insurance-capture step lands; scrub blocks if missing. Inbound rail (270/271 eligibility, 277 status, 835 ERA auto-post) still TODO. |
