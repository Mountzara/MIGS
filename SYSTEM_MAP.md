...
| `review_inserted_citations` | a per-citation model check on the exact sentence the marker ends (popovers masked; neighbours labelled context only), withdrawing **by position** | withdrawing by PMID removed fifteen correct instances of a paper cited once wrongly; handing the reviewer the 320 unmasked characters before the marker gave it the previous paper's hover card as "the sentence" and withdrew 27 correct citations in a chain |
...
`resynth`, `design`, `transform`, plus `kb` for the heading-area lookups. A regex fix after a failed check reruns the whole
