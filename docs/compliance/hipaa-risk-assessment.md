...
| 11 | DR — R2 PHI bucket loss | R2 object deletion | 2 | 4 | 8 | Cloudflare R2 multi-AZ durability (11 nines) + bucket lock `phi-hipaa-7yr` (2555-day retention — Cloudflare blocks DELETE on any object under retention, even from the operator's own API token). `mountzara-content` 1-year lock; `mountzara-backups` 90-day lock. Enabled 2026-05-16. | acceptable | operator | active |
