...
| R8 | Messaging response-window SLA | ✅ 2026-06-10 — migration `0022_phase18_messaging_sla.sql` (urgency/sla_due_at/sla_breached + partial index; apply BEFORE code deploy); `_lib/messaging.js::computeSlaDueAt` (America/Chicago business days, urgent=next-business-day 17:00 CT, non-urgent=2nd business day; functional test passed incl. Fri→Mon weekend skip + CST/CDT); patient thread-create requires `urgency` (`422→400 urgency_required`); patient reply restarts clock, clinician reply clears it; compose urgency radios with spec verbatim copy + 911 line; admin queue SLA badges (BREACHED red / urgent red / amber <24h / purple countdown); `mountzara-cron` 15-min sweep flags breaches + audit rows (**cron-worker needs its own `wrangler deploy`**). KNOWN GAP: no email/SMS notification — no mailer exists yet (deferred since Phase 2); breach surfacing = admin badge + audit until a mailer lands. `node --check` clean ×5. NOT deployed. |
| R9 | NPS post-visit survey | ✅ 2026-06-10 — migration `0023_phase18_nps.sql` (nps_dispatches + nps_responses; idempotent); internal dispatcher `/api/v1/internal/nps/dispatch` (X-Pipeline-Token; 36h completed-appt window, per-appt UNIQUE guard, 30-day patient cooldown, secure-message delivery from 'staff', audit rows); patient `/api/v1/patient/nps/respond` (token-is-auth, 14-day expiry → 410, idempotent overwrite); survey page `portal/nps/index.html` + `_redirects` wildcard (0–10 picker + why + /trending/ cross-promo thank-you); admin `nps/scores` + `nps/responses` (patient hash, 30/90/365 + response-rate sibling metric); analytics NPS cards; cron-worker `0 11 * * *` trigger. **Deploy prereqs: `wrangler secret put PIPELINE_TOKEN` on mountzara-cron + worker redeploy.** KNOWN GAP: email copy of the survey awaits a mailer (secure message + portal badge meanwhile). Backup TABLES list extended (0018+0023 tables; pre-existing 0006–0017 enumeration gap flagged in cron-worker/index.js). `node --check` clean ×7. NOT deployed. |
| R10 | Patient-facing visit summary, template fallback per D6 (migration 0024; draft/edit/send; portal view) | pending |
| R11 | Homepage telehealth value-prop + visit-type matrix | ✅ 2026-06-10 — new `#how-you-visit` section before `#curriculum` with spec verbatim copy + 14-row modality matrix hand-aligned to `visit_types.js` (honest four-bucket labels incl. adult-chaperone notes) + intake-assistant line + portal link. Inline Apple-glass recipe (no shared `<style>` edits on the danger-zone file). Verified: brace-balance 957/957, inline JS `node --check` OK, 0 blue tokens, 0 bare MIGS, 0 infra-language in rendered prose. §0.2.1 visual VERIFY at sprint-close deploy. |
...
**Route-render hard gate (2026-06-10, follow-up to the batch-1 bug class).** §0.2.1 visual VERIFY for page routes is now a MACHINE GATE, not discipline: `scripts/audit_route_render.py` + `scripts/route_render_manifest.json` run as `deploy-prod.sh` gate #8 — every manifest route is loaded in headless Chromium on live production and asserted (title contains, NOT the homepage title, DOM selector appears ≤15s), and the discovery contract fails the deploy if any static SPA route exists in the repo without a manifest entry (new pages cannot ship unregistered). Validated three ways: (a) 14/14 routes PASS standalone on live; (b) negative test — an unregistered `/portal/route-gate-negative-test/` route was correctly flagged as an orphan; (c) **in-chain green on deploy `f41fc41c`** after the gate first correctly BLOCKED a deploy (proving it has teeth) on what turned out to be transient network slowness — the audit now runs a retry-once pass so deterministic regressions (which fail twice) block while transient timeouts are absorbed (a flaky hard gate trains operators to skip it). Companion: smoketest §0b asserts `x-mz-route` headers on the dynamic Function-served routes. SYSTEM_MAP §2.3 (gates #7+#8) + §13.5 updated.
