...
- **SES event notifications (Bounce, Complaint, Delivery) publish to an
  SNS topic subscribed to an HTTPS endpoint in our application**
  (`https://mountzara.com/api/v1/internal/ses/feedback`), which maintains
  a suppression list in our database.
...
1. **Before sending, do the SNS setup if you haven't.** The reply states
   the SNS topic is subscribed to the feedback endpoint. The endpoint is
   deployed and live, but the topic subscription is a console step only
   you can do: SES (us-east-2) → verified identity `mountzara.com` →
   Notifications → publish Bounce/Complaint/Delivery to a new SNS topic →
   add an HTTPS subscription to
   `https://mountzara.com/api/v1/internal/ses/feedback`. Then set the
   topic's ARN as the `SES_SNS_TOPIC_ARN` Pages secret. The endpoint
   auto-confirms the subscription once the ARN matches. Five minutes, and
   it makes every sentence in section 3 literally true before a reviewer
   could check it.
